...
extern float position_target;	//设定目标位置
...
/**
  ...
  * @retval 无
  */
void set_p_i_d(float p, float i, float d, uint8_t ch)
{
	switch(ch){
		case 0x01://电流环q轴
				PID_current_q.P = p;    //航模电机，速度闭环，不能大于1，否则容易失控
				PID_current_q.I = i;    //电流环I参数不太好调试，只用P参数也可以
				PID_current_q.D = d;
			break;
		case 0x02://电流环q轴
				PID_current_d.P = p;    //航模电机，速度闭环，不能大于1，否则容易失控
				PID_current_d.I = i;    //电流环I参数不太好调试，只用P参数也可以
				PID_current_d.D = d;
			break;
		case 0x03://速度环
				PID_velocity.P = p;     //0.5
				PID_velocity.I = i;     //1
				PID_velocity.D = d;   
			break;
		case 0x04://位置环
...
/**
  ...
  * @retv0al 无
  */
void set_pid_target(float temp_val, uint8_t ch)
{
	switch(ch){
		case 0x01:
			targetIq = temp_val/1000;	
			break;
		case 0x02:
			targetId = temp_val/1000;
			break;
		case 0x03:
				velocity_target = temp_val/1000;	//待设定目标速度
			break;
		case 0x04://位置环目标值
				position_target = temp_val/1000;    // 设置当前的目标值
			break;
		default:
			break;
	}
...
//  uint8_t frame_data[128];         // 要能放下最长的帧
  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
				uint8_t tempch = frame_data[4];//通道地址
        uint32_t temp0 = COMPOUND_32BIT(&frame_data[13]);
        uint32_t temp1 = COMPOUND_32BIT(&frame_data[17]);
        uint32_t temp2 = COMPOUND_32BIT(&frame_data[21]);
        ...
        set_p_i_d(p_temp, i_temp, d_temp, tempch);    // 设置 P I D
      }
      break;
      ...
      case SET_TARGET_CMD:
      {
        ...
        set_pid_target(actual_temp, tempch);    // 设置目标值
      }
      break;
